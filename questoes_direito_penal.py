# -*- coding: utf-8 -*-
"""
100 Questões de Direito Penal para Agente de Trânsito
Estilo: Fundatec/Fepese - Foco em literalidade da lei e situações práticas
Temas: Crimes contra a Administração Pública, Crimes contra a Fé Pública e Abuso de Autoridade
"""
...
def obter_questoes_por_categoria(categoria):
    """Retorna todas as questões de uma categoria específica"""
    return [q for q in QUESTOES if q["categoria"] == categoria]

def obter_categorias():
    """Retorna lista de categorias únicas"""
    return list(set([q["categoria"] for q in QUESTOES]))

def obter_total_questoes():
    """Retorna o total de questões"""
    return len(QUESTOES)

# Teste
if __name__ == "__main__":
    print(f"Total de questões: {obter_total_questoes()}")
    print(f"\nCategorias disponíveis:")
    for cat in obter_categorias():
        total = len(obter_questoes_por_categoria(cat))
        print(f"  - {cat}: {total} questões")
